 ...
 * All filters are optional; combined with AND. Text `q` matches name, city, state, EIN, mission.
 */

export type Irs990SearchEnv = { DB: D1Database };

export type Irs990SearchFilters = {
  /** Broad text: org name, city, state, EIN digits, mission/activity text */
  q?: string;
  /** US state 2-letter */
  state?: string;
  /** City substring */
  ...
  has_website?: boolean;
  sort?: "name" | "revenue" | "expenses" | "tax_yr" | "ein";
  limit?: number;
  offset?: number;
};
...
export function parseSearchFilters(input: URLSearchParams | Record<string, unknown>): Irs990SearchFilters {
  ...
  const rvmin = num(get("revenue_min_usd"));
  const rvmax = num(get("revenue_max_usd"));
  if (rvmin !== undefined) o.revenue_min_usd = rvmin;
  if (rvmax !== undefined) o.revenue_max_usd = rvmax;
  const exmin = num(get("expenses_min_usd"));
  const exmax = num(get("expenses_max_usd"));
  if (exmin !== undefined) o.expenses_min_usd = exmin;
  if (exmax !== undefined) o.expenses_max_usd = exmax;
  const namin = num(get("net_assets_min_usd"));
  const namax = num(get("net_assets_max_usd"));
  if (namin !== undefined) o.net_assets_min_usd = namin;
  if (namax !== undefined) o.net_assets_max_usd = namax;
  const grmin = num(get("gross_receipts_min_usd"));
  const grmax = num(get("gross_receipts_max_usd"));
  if (grmin !== undefined) o.gross_receipts_min_usd = grmin;
  if (grmax !== undefined) o.gross_receipts_max_usd = grmax;
  const psemin = num(get("program_service_expenses_min_usd"));
  const psemax = num(get("program_service_expenses_max_usd"));
  if (psemin !== undefined) o.program_service_expenses_min_usd = psemin;
  if (psemax !== undefined) o.program_service_expenses_max_usd = psemax;
  ...
  if (sort === "name" || sort === "revenue" || sort === "expenses" || sort === "tax_yr" || sort === "ein") {
    o.sort = sort;
  }
...
export async function runIrs990Search(
  ...
  const stmt = `
    SELECT r.return_pk, r.ein, r.org_legal_name AS name, r.filer_city AS city, r.filer_state AS state,
           ...
           r.total_program_service_expenses_amt, r.cy_total_fundraising_expense_amt,
