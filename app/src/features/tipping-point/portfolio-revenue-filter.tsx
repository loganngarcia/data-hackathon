...
type Props = {
  value: RevenueBandId;
  onChange: (band: RevenueBandId) => void;
};
...
/** Latest-year revenue band filter — same floating menu pattern as other portfolio toolbar pickers. */
export function PortfolioRevenueFilter(props: Props) {
  const { value, onChange } = props;

  const options = useMemo(
    () => REVENUE_BAND_IDS.map((id) => ({ id, label: REVENUE_BAND_LABELS[id] })),
    [],
  );

  const bandLabel = options.find((o) => o.id === value)?.label ?? "";
  /** Chip preview: e.g. "Over $5M revenue" — dropdown labels unchanged. */
  const triggerLabel =
    value === "all" ? "Revenue" : `${bandLabel} revenue`;
  const ariaFilterDetail =
    value === "all" ? "all revenue ranges" : `${REVENUE_BAND_LABELS[value] ?? ""} revenue`;
  ...
  const menu =
    open && (isMobile || fixedPos) && options.length > 0 ? (
      <>
        ...
        />
        <div
          data-layer="portfolio revenue menu"
    ...
    <div className="tp-portfolio-revenue-select-wrap">
      <div className="tp-portfolio-revenue-picker tp-panel-compare-peer-picker">
        <button
          ...
          aria-expanded={open}
          aria-label={`Revenue filter: ${ariaFilterDetail}. Open menu.`}
          onClick={() => {
            if (open) {
              setOpen(false);
              return;
