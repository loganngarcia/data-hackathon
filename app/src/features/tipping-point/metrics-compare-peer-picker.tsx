...
function ChevronDown({ open }: { open: boolean }) {
  return (
    <span
      className="tp-org-chip-chevron tp-panel-compare-peer-chevron"
      aria-hidden
      style={{
        display: "flex",
        alignItems: "center",
...
export type MetricsCompareOption = { id: string; label: string };

type Props = {
  options: MetricsCompareOption[];
  value: string;
  onChange: (id: string) => void;
};

export function MetricsComparePeerPicker({ options, value, onChange }: Props) {
  ...
  const menu =
    open && (isMobile || fixedPos) && options.length > 0 ? (
      <>
        ...
        />
        <div
          data-layer="metrics compare peer menu"
  ...
  return (
    <div className="tp-panel-compare-peer-picker">
      <button
        ref={chipRef}
        type="button"
        ...
        aria-expanded={open}
        aria-label={`Compare against: ${selectedLabel}. Open menu.`}
        onClick={() => {
          if (open) {
            setOpen(false);
            return;
      ...
      >
        <span className="tp-panel-compare-peer-trigger-label">{selectedLabel}</span>
