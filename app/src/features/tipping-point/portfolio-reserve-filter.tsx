...
type Props = {
  value: ReserveBandId;
  onChange: (band: ReserveBandId) => void;
};
...
/** Reserve coverage (months of net assets ÷ CY expenses) — same menu pattern as revenue. */
export function PortfolioReserveFilter(props: Props) {
  const { value, onChange } = props;

  const options = useMemo(
    () => RESERVE_BAND_IDS.map((id) => ({ id, label: RESERVE_BAND_LABELS[id] })),
    [],
  );

  const bandLabel = options.find((o) => o.id === value)?.label ?? "";
  /** Chip preview: e.g. "3–6 months reserve" (dropdown rows unchanged). */
  const triggerLabel =
    value === "all" ? "Reserve" : `${bandLabel} reserve`;
  const ariaFilterDetail =
    value === "all"
      ? "all reserve ranges"
      : `${RESERVE_BAND_LABELS[value] ?? ""} reserve`;
  ...
  const menu =
    open && (isMobile || fixedPos) && options.length > 0 ? (
      <>
        ...
        />
        <div
          data-layer="portfolio reserve menu"
    ...
    <div className="tp-portfolio-revenue-select-wrap">
      <div className="tp-portfolio-revenue-picker tp-panel-compare-peer-picker">
        <button
          ...
          aria-expanded={open}
          aria-label={`Reserve filter: ${ariaFilterDetail}. Open menu.`}
          onClick={() => {
            if (open) {
              setOpen(false);
              return;
