...
const emptyProfile = (): YouProfileSnapshot => ({
    name: "",
    jobTitle: "",
    favoriteThings: "",
})
...
export function YouSettingsOverlay({ open, isMobile, onClose }: Props) {
    /** Match SSR and first client paint; `useLayoutEffect` loads localStorage before paint. */
    const [profile, setProfile] = useState<YouProfileSnapshot>(emptyProfile)
    ...
    useEffect(() => {
        if (!open || !storageHydrated) return
        saveYouProfile(profile)
    }, [open, profile, storageHydrated])
