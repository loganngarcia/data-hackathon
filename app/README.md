...
This directory is the **Next.js 15 + TypeScript** demo for **Tipping Point**. The **main experience** (`/`) ports the **`data-hackathon-dashboard` shell**: left sidebar (chats, search, new chat, You), bottom **chat composer**, `?settings` profile sheet, and chat sessions at **`/c/[chatId]`**. The center column is the **mosaic dashboard**: on load it fetches **`/api/portfolio-data`** (Cloudflare Worker TEOS / D1 `irs990_xml_returns` + **Moobu-style** 0–100 resilience scores). If the Worker is unavailable, the route returns an error (no placeholder portfolio).
...
| **`/legacy`** | Short notice; live dashboard is **`/`** (the old static deck was removed). |
...
| **`types.ts`**, **`format-display.ts`** | Shared types and formatting for the mosaic. |
...
- **`NEXT_PUBLIC_LOGO_DEV_PUBLISHABLE_KEY`** — optional local fallback for [Logo.dev](https://www.logo.dev/) avatars; production prefers **`LOGO_DEV_PUBLISHABLE_KEY`** on the Cloudflare Worker (`logo_image_url` on browse rows).
